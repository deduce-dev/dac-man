...
def start_stats_calculation(data_datablock_send_start_dir,
    ...
    for key, value in data_datablock_send_start.items():
           data_send_end_max = max(float(data_task_send_end[key]), data_send_end_max)
           data_send_end_min = min(float(value), data_send_end_min)

    data_send_end_diff = data_send_end_max - data_send_end_min
    print("Job arrival time period [Max(data_task_send_end) - Min(data_datablock_send_start)]: ", 
    ...
    for _, value in data_datablock_send_start.items():
           data_send_start_min = min(float(value), data_send_start_min)

    turaround_time = job_end_data_put_max - data_send_start_min
    print("Turnaround time [Max(job_end_data_put) - Min(data_datablock_send_start)]: ", 
    ...
    for key, time in job_end_data_put_sorted:
        diff_value = float(job_end_data_put[key]) - float(data_datablock_send_end[key])
    ...
    print("Event-time Latency [Avg(job_end_data_put - data_datablock_send_end)]: ", 
    ...
    #### Calculating Data transfer overhead [Avg(data_task_send_end - data_datablock_send_start)]

    data_transfer_overhead_list = []
    for key, time in data_send_start_sorted:
        diff_value = float(data_task_send_end[key]) - float(data_datablock_send_start[key])
        data_transfer_overhead_list.append(diff_value)

    data_transfer_overhead = np.mean(data_transfer_overhead_list)
    print("Data transfer overhead [Avg(data_task_send_end - data_datablock_send_start)]: ", 
...
def main(argv):
    ...
    start_stats_calculation(data_datablock_send_start_dir,
                            data_datablock_send_end_dir,
                            data_task_send_start_dir,
                            data_task_send_end_dir,
                            data_pull_start_dir,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 4:
        ...
        print("Usage: python calc_stats_per_setup.py <sources> <workers> <output_dir>")
        exit()
